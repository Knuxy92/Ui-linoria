	...
	do -- Custom Ui
		local UiStorke = Instance.new("UIStroke", ScreenGui.Assets.Elements.Toggle.TextLabel.TextButton.Background)
		UiStorke.Color = Color3.fromRGB(88,88,88)

		local UiStorke = Instance.new("UIStroke", ScreenGui.Assets.Elements.TextBox.TextLabel.TextBox)
		UiStorke.Color = Color3.fromRGB(88,88,88)
	...
	ShareToScript.Flags = Library.Flags
	...
	function Library:createToggle(options: table, parent, scrollingFrame)
		...
		local Context = Utility:validateContext({
			...
			switchOff = {Value = function()
				tweenToggleAssets(Theme.SecondaryBackgroundColor, Color3.fromRGB(45, 45, 45), Vector2.new(0, 0.5), UDim2.fromScale(0, 0.5), 0.2)
				circleOn = false
			end, ExpectedType = "function"},
		...
		ShareToScript.Flags.Toggle[options.text] = {
	...
	function Library:createSlider(options: table, parent, scrollingFrame)
		...
		ShareToScript.Flags.Slider[options.text] =  {
	...
	function Library:createPicker(options: table, parent, scrollingFrame, isPickerBoolean)
		...
		ShareToScript.Flags.ColorPicker[options.text] = {
	...
	function Library:createDropdown(options: table, parent, scrollingFrame)
		...
		ShareToScript.Flags.Dropdown[options.text] = {
			getList = function()
				return Context.list
			end,
	...
	function Library:createKeybind(options: table, parent, scrollingFrame)
		...
		ShareToScript.Flags.Keybind[options.text] = {
	...
	function Library:createTextBox(options: table, parent, scrollingFrame)
		...
		ShareToScript.Flags.TextBox[options.text] = {
	...
	function Library:createManager(options: table)
		...
		local function getSavedData()
			...
			for elementType, elementData in pairs(ShareToScript.Flags) do
			...
			for elementType, elementData in pairs(ShareToScript.Flags) do
				for elementName, _ in pairs(elementData) do
					if elementType == "Dropdown" and decoded.Dropdown[elementName] and elementName ~= "Configs" then
						ShareToScript.Flags.Dropdown[elementName]:updateList({list = decoded.Dropdown.list, default = decoded.Dropdown.value})
					end

					if elementType == "Toggle" and decoded.Toggle[elementName] then
						ShareToScript.Flags.Toggle[elementName]:updateState({state = decoded.Toggle[elementName].state})
					end

					if elementType == "Slider" and decoded.Slider[elementName] then
						ShareToScript.Flags.Slider[elementName]:updateValue({value = decoded.Slider[elementName].value})
					end

					if elementType == "Keybind" and decoded.Keybind[elementName] then
						ShareToScript.Flags.Keybind[elementName]:updateKeybind({bind = decoded.Keybind[elementName].keybind})
					end

					if elementType == "TextBox" and decoded.TextBox[elementName] then
						ShareToScript.Flags.TextBox[elementName]:updateText({text = decoded.TextBox[elementName].text})
					end

					if elementType == "ColorPicker" and decoded.ColorPicker[elementName] then
						ShareToScript.Flags.ColorPicker[elementName]:updateColor({color = Color3.fromRGB(unpack(decoded.ColorPicker[elementName].color))})
					end
				end
			end
		end
		...
		SaveManager:createButton({text = "Create Config", callback = function()
			...
			if ShareToScript.Flags.Dropdown["Configs"] then
				ShareToScript.Flags.Dropdown["Configs"]:updateList({list = getJsons(), default = {ShareToScript.Flags.Dropdown["Configs"]:getValue()}})
			end
		end})
		...
		ThemeManager:createButton({
			...
			callback = function()
				...
				if ShareToScript.Flags.Dropdown["Theme Configs"] then
					ShareToScript.Flags.Dropdown["Theme Configs"]:updateList({list = getThemeJsons(), default = {ShareToScript.Flags.Dropdown["Theme Configs"]:getValue()}})
				end
			end
		})
